...
const DOMAIN = {
  ...
  trust_collapse: 'social', truth_vs_rumour: 'social',
...
app.post('/v1/simulate', async (req, reply) => {
  ...
  try {
    ...
    app.log.error(err);
    return reply.send({ templateId: 'virus_infecting_cell', confidence: 0.5, parameterOverrides: {}, narration: [{ tick: 0, text: 'Watch how this biological process unfolds.' }, { tick: 800, text: 'Entities interact according to causal rules.' }, { tick: 1500, text: 'This mechanism drives the real-world process.' }], fallback: true, domain: 'biology' });
  }
});
